...
void wigglerM(struct gwig *pWig, double* orbit_in, double L,double *M66)
{
	int i,j,k;
	double H2[36];
	double H[6][6];
...
{	
    ...
    //DLDS =1 + ((D +1)/( sqrt( (D+1)*(D+1) - (px-ax)*(px-ax) -(py-ay)*(py-ay))));
     //       (((px-ax)*(px-ax) + (py-ay)*(py-ay))/(2*(1+D)*(1+D))); 
    DLDS=L*SQR(SQR(1+orbit_in[4]))*(1+((px-ax)*(px-ax)/(2*(1+D)*(1+D)))+((py-ay)*(py-ay)/(2*(1+D)*(1+D))));      
	//DLDS=L*SQR(SQR(1+orbit_in[4]))*(1+((px-ax)*(px-ax)/(2*(1+D)*(1+D)))+((py-ay)*(py-ay)/(2*(1+D)*(1+D))));
	//DLDS=((SQR(px-ax)+SQR(py-ay))/(4*SQR(1+D)));
